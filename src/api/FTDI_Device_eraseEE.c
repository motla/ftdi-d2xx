...
static void execute_callback(napi_env env, void* data) {
  ...
  // Erase FTDI device EEPROM
  async_data->ftStatus = FT_EraseEE(async_data->instance_data->ftHandle);
}
...
napi_value device_eraseEE(napi_env env, napi_callback_info info) {
  ...
  napi_value name;
  utils_check(napi_create_string_utf8(env, "deviceEraseEE", NAPI_AUTO_LENGTH, &name));
