...
napi_value device_setDataCharacteristics(napi_env env, napi_callback_info info) {
  ...
  if(utils_check(
      ...
      "Number of bits per word must be FT_BITS_8 or FT_BITS_7", "wrongarg")) return NULL;
  if(utils_check(
      ((napi_get_value_uint32(env, argv[1], &stop_bits) != napi_ok) || (stop_bits != FT_STOP_BITS_1 && stop_bits != FT_STOP_BITS_2)),
      "Number of stop bits must be FT_STOP_BITS_1 or FT_STOP_BITS_2", "wrongarg")) return NULL;
  if(utils_check(
