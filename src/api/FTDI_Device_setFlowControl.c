...
napi_value device_setFlowControl(napi_env env, napi_callback_info info) {
  ...
  if(utils_check(argc < 3, "Missing argument", "missarg")) return NULL;
  ...
  if(utils_check(
      ...
      "Flow control must be FT_FLOW_NONE, FT_FLOW_RTS_CTS, FT_FLOW_DTR_DSR or FT_FLOW_XON_XOFF", "wrongarg")) return NULL;
  if(flow == FT_FLOW_XON_XOFF) {
    if(utils_check(((napi_get_value_uint32(env, argv[1], &xon) != napi_ok) || xon > 0xFF),
        "Byte to signal Xon must be a byte number (0-255)", "wrongarg")) return NULL;
