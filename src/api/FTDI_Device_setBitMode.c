...
napi_value device_setBitMode(napi_env env, napi_callback_info info) {
  ...
  if (utils_check(argc < NB_ARGS, "Missing argument", "missarg"))
    return NULL;
  ...
  if (utils_check(instance_data->ftHandle == NULL, "Dead device object", "deadobj"))
    return NULL;
  ...
  if (utils_check(
          ...
          "Mask must be a number and less than 0xFF", "wrongarg"))
    return NULL;
  if (utils_check(
          ((napi_get_value_uint32(env, argv[1], &bit_mode) != napi_ok) || (bit_mode != FT_BITMODE_RESET &&
                                                                           bit_mode != FT_BITMODE_ASYNC_BITBANG &&
                                                                           bit_mode != FT_BITMODE_MPSSE &&
                                                                           bit_mode != FT_BITMODE_SYNC_BITBANG &&
                                                                           bit_mode != FT_BITMODE_MCU_HOST &&
                                                                           bit_mode != FT_BITMODE_FAST_SERIAL &&
                                                                           bit_mode != FT_BITMODE_CBUS_BITBANG &&
                                                                           bit_mode != FT_BITMODE_SYNC_FIFO)),
          "Bit mode must be FT_BITMODE_RESET, FT_BITMODE_ASYNC_BITBANG, FT_BITMODE_MPSSE, FT_BITMODE_SYNC_BITBANG, FT_BITMODE_MCU_HOST, FT_BITMODE_FAST_SERIAL, FT_BITMODE_CBUS_BITBANG or FT_BITMODE_SYNC_FIFO", "wrongarg"))
    return NULL;
