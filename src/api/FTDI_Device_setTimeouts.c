...
napi_value device_setTimeouts(napi_env env, napi_callback_info info) {
  ...
  if(utils_check(napi_get_value_uint32(env, argv[0], &read_timeout), "Read timeout must be a number", ERR_WRONGARG)) return NULL;
  if(utils_check(napi_get_value_uint32(env, argv[1], &write_timeout), "Write timeout must be a number", ERR_WRONGARG)) return NULL;
